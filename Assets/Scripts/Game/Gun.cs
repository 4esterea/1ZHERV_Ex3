...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        if (shotgun)
        {
            // Calculate the start angle for the spread.
            float startAngle = -shotgunSpread / 2;

            // Calculate the angle step for each bullet.
            float angleStep = shotgunSpread / (shotgunBullets - 1);

            for (int i = 0; i < shotgunBullets; i++)
            {
                // Calculate the rotation for this bullet.
                Quaternion bulletRotation = Quaternion.Euler(0.0f, 0.0f, startAngle + angleStep * i);

                // Spawn the bullet.
                SpawnBullet(director.position, director.rotation * bulletRotation);
            }
        }
        else
        {
            // Spawn a single bullet in the direction of the director.
